<template>
  <div class="anotacoes-adm">
    <TituloCadastro
      icon="fa fa-pencil-square-o fa-lg"
      titulo="ADM > Cadastro de Anotações"
          ...
          <b-input-group size="md">
            ...
            <b-input-group-append>
              <b-button
                type="button"
                :disabled="!filterTemp"
                @click="filterTemp='';loadAnotacoesAdm();"
              >X</b-button>
            </b-input-group-append>
          </b-input-group>
        </b-col>
...
import AnotacaoExibir from "../../pages/components/AnotacaoExibir";
...
export default {
  ...
  methods: {
    loadAnotacoesAdm() {
      const url = `${baseApiUrl}/adm_anotacao/?page=1`;
      axios
    ...
    keyUpFilter() {
      ...
      this.timeFilter = setTimeout(() => {
        const url = `${baseApiUrl}/adm_anotacao/?page=${this.page}`;
        axios
          .put(url, { filtro: this.filterTemp })
          .then((res) => {
            this.totalAnotacoes = res.data.total_registros.total_registros;
...
.anotacoes-adm .row-btn {
  display: flex;
  justify-content: center;
  width: 100%;
}
.anotacoes-adm .row-btn button {
  width: 23%;
}
.anotacoes-adm .row-btn button:last-child {
  margin-left: 25px;
}
